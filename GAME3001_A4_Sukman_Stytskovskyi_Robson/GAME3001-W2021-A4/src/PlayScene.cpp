...
{
		// Skeleton off screen 
	if (m_pSkeletonRanged->getTransform()->position.x < 0 || m_pSkeletonRanged->getTransform()->position.x > 800
		|| m_pSkeletonRanged->getTransform()->position.x < 0 || m_pSkeletonRanged->getTransform()->position.x > 600)
	{
		m_pSkeletonRanged->setOffscreenCounter(m_pSkeletonRanged->getOffscreenCounter() + 1);
	}
	if (m_pSkeletonRanged->getOffscreenCounter() > 300)
	{
		resetRangedSkeleton();
	}

	if(m_pSkeletonClose->getTransform()->position.x < 0 || m_pSkeletonClose->getTransform()->position.x > 800 ||
		m_pSkeletonClose->getTransform()->position.y < 0 || m_pSkeletonClose->getTransform()->position.y > 600)
	{
		m_pSkeletonClose->setOffscreenCounter(m_pSkeletonClose->getOffscreenCounter() + 1);
	}
	if (m_pSkeletonClose->getOffscreenCounter() > 300)
	{
		resetCloseSkeleton();
	}
...
void PlayScene::resetRangedSkeleton()
{
	auto offset = glm::vec2(Config::TILE_SIZE * 0.5f, Config::TILE_SIZE * 0.5f);

	m_pSkeletonRanged->getHealthBar().setHealthPoints(65);
	m_pSkeletonRanged->setWaitInCoverTimer(0);
	m_pSkeletonRanged->setTimerNotOut(true);
	m_pSkeletonRanged->setIsBehindCover(false);
	m_pSkeletonRanged->getTransform()->position = m_getTile(1, 1)->getTransform()->position + offset;
	m_pSkeletonRanged->setAnimationState("WalkingRight");
	m_pSkeletonRanged->setMaxSpeed(2.0f);
	m_pSkeletonRanged->setTakingDamage(false);
	m_pSkeletonRanged->setFleeing(false);
	m_pSkeletonRanged->setCurrentAction("Patrol");
	m_pSkeletonRanged->setOffscreenCounter(0);
}

void PlayScene::resetCloseSkeleton()
{
	m_pSkeletonClose->getHealthBar().setHealthPoints(135);
	auto offset = glm::vec2(Config::TILE_SIZE * 0.5f, Config::TILE_SIZE * 0.5f);
	m_pSkeletonClose->getTransform()->position = m_getTile(5, 4)->getTransform()->position + offset;
	m_pSkeletonClose->setAnimationState("WalkingDown");
	m_pSkeletonClose->setMaxSpeed(2.0f);
	m_pSkeletonClose->setTakingDamage(false);
	m_pSkeletonClose->setFleeing(false);
	m_pSkeletonClose->setCurrentAction("Patrol");
	m_pSkeletonClose->setOffscreenCounter(0);
}
